...
namespace SampleContentControl.ViewModels
{
	public class UsersViewModel : ViewModelBase
	{
		...
		{
			Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: UsersViewModel: OnActivated({++_activatedCount})");

			try
			{

				Disposable
					.Create(() => Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: UsersViewModel: Deactivated."))
					.DisposeWith(d);
			}
			catch (Exception ex)
			{
				string msg = $"{ex.Message}:{Environment.NewLine}{ex.StackTrace}";
				Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: UsersViewModel: Exception: ({msg})");
			}
