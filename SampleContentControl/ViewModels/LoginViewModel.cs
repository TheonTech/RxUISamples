...
namespace SampleContentControl.ViewModels
{
	public class LoginViewModel : ViewModelBase
	{
		...
		{
			Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: LoginViewModel: OnActivated({++_activatedCount})");

			try
			{

				Disposable
					.Create(() => Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: LoginViewModel: Deactivated."))
					.DisposeWith(d);
			}
			catch (Exception ex)
			{
				string msg = $"{ex.Message}:{Environment.NewLine}{ex.StackTrace}";
				Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: LoginViewModel: Exception: ({msg})");
			}
