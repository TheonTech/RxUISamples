...
namespace SampleViewModelViewHost.ViewModels
{
	public class PackagesViewModel : ViewModelBase
	{
		...
		{
			Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: PackagesViewModel: OnActivated({++_activatedCount})");

			try
			{

				Disposable
					.Create(() => Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: PackagesViewModel: Deactivated."))
					.DisposeWith(d);
			}
			catch (Exception ex)
			{
				string msg = $"{ex.Message}:{Environment.NewLine}{ex.StackTrace}";
				Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: PackagesViewModel: Exception: ({msg})");
			}
