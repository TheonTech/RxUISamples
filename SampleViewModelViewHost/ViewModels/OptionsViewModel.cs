...
namespace SampleViewModelViewHost.ViewModels
{
	public class OptionsViewModel : ViewModelBase
	{
		...
		{
			Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: OptionsViewModel: OnActivated({++_activatedCount})");

			try
			{

				Disposable
					.Create(() => Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: OptionsViewModel: Deactivated."))
					.DisposeWith(d);
			}
			catch (Exception ex)
			{
				string msg = $"{ex.Message}:{Environment.NewLine}{ex.StackTrace}";
				Debug.WriteLine($"{DateTime.Now:dd HH:mm:ss.fffff}: OptionsViewModel: Exception: ({msg})");
			}
